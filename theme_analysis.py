...
OUTPUT_HOT_WL    = "txt/hot_theme_watchlist.txt"  # ★ 新增
RATING_THRESHOLD = 6
...
# ── Phase 1：建立今日熱門題材 ─────────────────────────────────────────────
def fetch_hot_themes(client: genai.Client, rs95_tickers: list[tuple]) -> tuple[str, dict, list]:
    # ★ 回傳值改為 (themes_text, ticker_themes, hot_themes_ordered)
    ...
    # ★ ticker 清單也拿掉 RS 標注，只留 ticker
    ticker_lines = "\n".join(f"  {t}" for t, rs in rs95_tickers)
...
分析說明：
- 這些是全市場相對強度最高且流動性充足的股票
- 請依股票在清單中的排序加權判斷題材熱度：排序越前面的股票代表強度越高，權重越大
- 多檔排序靠前的股票集中同一板塊，代表該題材資金極度集中，應列為頂級熱門
- 一檔個股可歸屬多個題材
...
- 題材用繁體中文，力求精準"""
    ...
    rs_lookup = {t: rs for t, rs in rs95_tickers}  # ★ 保留 rs_lookup 供後續排序用
    ...
    for attempt in range(5):
        try:
            ...
            # 題材名稱順序（用於分組排序）
            # ★ 從 hot_themes_list 抽出純題材名稱（— 前面的部分）
            hot_theme_names_ordered = []
            for item in hot_themes_list:
                name = item.split("—")[0].strip()
                hot_theme_names_ordered.append(name)
            ...
            for ticker_upper, themes in ticker_themes.items():
                ...
                for theme in themes:
                    theme_count.setdefault(theme, []).append(t)  # ★ 只存 ticker
            ...
            # ★ 同時回傳 ticker_themes 和排序好的題材名稱，供主流程產生 hot_theme_watchlist
...
def build_hot_theme_watchlist(
    ...
    回傳 ticker list。
    """
    # 為每個 ticker 找到它應該歸屬的「熱度最高的題材」
    # hot_theme_names_ordered[0] 是最熱門的題材
    ...
    for ticker_raw, themes in ticker_themes.items():
        ...
        if not themes:
            continue
        # 找這個 ticker 所有題材中，熱度排名最高（index 最小）的那個
        best = min(
